...
enum class ColorErrorCode { InvalidArgument, OutOfRange, NotThreeTokens };

std::variant<std::array<std::string, 3>, ColorErrorCode>
splitColor(std::string const &s) {
...
std::variant<Color, ColorErrorCode>
colorsStringArrToStruct(std::array<std::string, 3> const &tokens) {
  uint8_t value;
...
void changeColor(IController::Ptr const &c_ptr) {
  ...
  if (std::holds_alternative<std::array<std::string, 3>>(res)) {
    auto const res2 =
        colorsStringArrToStruct(std::get<std::array<std::string, 3>>(res));
    if (std::holds_alternative<Color>(res2)) {
      c_ptr->setColor(std::get<Color>(res2));
    } else if (std::get<ColorErrorCode>(res2) ==
               ColorErrorCode::InvalidArgument) {
      std::cout << "INVALID ARGUMENT\n";
    ...
    std::cout << "UNABLE TO GET THREE COLORS: EXPECTED THREE TOKENS SEPARATED "
                 "BY DOT\n";
  }
}
