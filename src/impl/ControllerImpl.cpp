...
void buildReport(unsigned char *out_buf,
...
void sendFeatureReport(hid_device *const dev,
                       unsigned char const (&payload)[kReportLength]) {
...
void ControllerImpl::setBrightness(uint16_t const lvl) const {
  ...
  unsigned char payload[kReportLength]{}; // use unsigned char because of C
                                          // library, that uses this type
  buildReport(payload,
              {kBrightnessByte, kActionByte, static_cast<unsigned char>(lvl)});
...
void ControllerImpl::setColor(Color const Color) const {
  ...
  buildReport(payload, {kColorByte,
                        0x01_uc, // control
                        0x01_uc, // row
