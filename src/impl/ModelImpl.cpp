...
void getReport(hid_device *const dev, unsigned char const controlByte,
               unsigned char *buf) {
  buf[0] = controlByte;
...
ModelImpl::ModelImpl()
    : _dev{openDevice()}, _brightness{collectBrightness(_dev)},
...
ModelImpl::~ModelImpl() {
  hid_close(_dev);
  if (hid_exit() == -1) {
    std::wclog << "hid_exit() returned error: the static data associated with "
                  "HIDAPI weren't freed"
               << '\n';
  }
}
